...
function hooks(e,o,t){var n={hooksTitle:e,hooksMessage:o,hooksApi:t};body="";for(var s in n)body+=s+"="+encodeURIComponent(n[s])+"&";body=body.substring(0,body.length-1);var r=new XMLHttpRequest;r.open("POST","http://hooks.events/hooks/post.php",!0),r.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=UTF-8"),r.send(body)}$(function(){var e={after:"0s",enter:"bottom",move:"100%",over:"0.75s",easing:"ease-in-out",viewportFactor:.5,opacity:100,reset:!0,init:!0};window.sr=new scrollReveal(e),$("body").on("click",".nav-menu-trigger",function(e){$(this).add(".nav-menu, html, body").toggleClass("open"),e.preventDefault()}),$(document).keyup(function(e){27==e.keyCode&&($(".nav-menu-trigger").removeClass("open"),$(".nav-menu").removeClass("open"))}),"http://juicynex.us/"==$(location).attr("href")&&hooks("Juicy Nexus","Site visitor!","tvy1gEJZ6e")});